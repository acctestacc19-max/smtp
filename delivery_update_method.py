...
def parse_args(apwx: Apwx) -> Apwx:
    ...
    parser.add_arg(str(AppWorxEnum.RUN_DATE),
                   type=lambda d: datetime.datetime.strptime(d, '%m-%d-%Y').strftime('%m-%d-%Y'),
                   required=False)
    ...
    parser.add_arg(str(AppWorxEnum.EMAIL_RECIPIENTS), type=r'([\w\.]+@firsttechfed\.com,?)+', ignore_case=True,
                   required=True)
    ...
    parser.add_arg(str(AppWorxEnum.FROM_EMAIL_ADDR), type=r'[\w\.]+@firsttechfed\.com',
                   default='AM_PROD@firsttechfed.com',
...
def process_records(script_data: ScriptData, pers_records: list[dict], org_records: list[dict]) -> tuple[list, list]:
    ...
    p_successes, p_fails = update_stdl_userfield(script_data, pers_records, table_name='persuserfield',
                                                 col_name='persnbr')
...
def update_stdl_userfield(script_data: ScriptData, records: list[dict], table_name: str, col_name: str) -> tuple[
    list, list]:
